// underground pass (members) object interactions with quest-state logic.
// wells, gates, cages, chests, the zamorakian temple door and railings.
// tile navigation lives in ./obstacles.js; the timed iban's chamber blast is
// startIbanChamberHazard below.
...
// iban's chamber blast: each tick blasts a random tile; ends after 2 min in the
// bound area (ejects), on leaving the area, or when iban is defeated.
const IBAN_CHAMBER_BOUND = { minX: 794, maxX: 800, minY: 3467, maxY: 3471 };
const IBAN_CHAMBER_LEAVE_BOUND = {
    minX: 794,
    maxX: 804,
    minY: 3464,
    maxY: 3474
};
const IBAN_CHAMBER_TIMEOUT_MS = 2 * 60 * 1000;

function inBound(x, y, area) {
    return x >= area.minX && x <= area.maxX && y >= area.minY && y <= area.maxY;
}

function startIbanChamberHazard(player) {
    const { world } = player;
    const start = Date.now();

    const tick = () => {
        if (!player.loggedIn) {
            return; // stop: logged out
        }
        if (
            Date.now() - start > IBAN_CHAMBER_TIMEOUT_MS &&
            inBound(player.x, player.y, IBAN_CHAMBER_BOUND)
        ) {
        ...
        if (!inBound(player.x, player.y, IBAN_CHAMBER_LEAVE_BOUND)) {
            return; // stop: left the chamber
        }
        if (player.cache.iban_bubble_show) {
            return; // stop: iban defeated
        }

        const blastX =
            IBAN_CHAMBER_BOUND.minX +
            Math.floor(
                Math.random() *
                    (IBAN_CHAMBER_BOUND.maxX - IBAN_CHAMBER_BOUND.minX + 1)
            );
        const blastY =
            IBAN_CHAMBER_BOUND.minY +
            Math.floor(
                Math.random() *
                    (IBAN_CHAMBER_BOUND.maxY - IBAN_CHAMBER_BOUND.minY + 1)
            );
        player.sendTeleportBubble(blastX, blastY, 'telegrab');

        if (Math.abs(player.x - blastX) <= 1 && Math.abs(player.y - blastY) <= 1) {
            const ibanDmg =
                Math.floor(player.skills.hits.current / 10) +
                4 +
                (Math.floor(Math.random() * 3) - 1);
            const willDie = ibanDmg >= player.skills.hits.current;
            player.damage(ibanDmg);
            if (!willDie) {
...
async function onGameObjectCommandOne(player, gameObject) {
    ...
    if (gameObject.id === IDS.WELL) {
        player.message('@que@you climb into the well');
        ...
        if (orbsDone || stage === 7 || stage === 8 || stage === -1) {
            player.message('@que@you feel the grip of icy hands all around you...');
            await world.sleepTicks(3);
            ...
            player.sendTeleportBubble(gameObject.x, gameObject.y);
            player.message('@que@from below an icy blast of air chills you to your bones');
            await world.sleepTicks(3);
            player.message('@que@a mystical force seems to blast you back out of the well');
            await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.CAGE_REMAINS) {
        if (stage >= 5 || stage === -1) {
            player.message('@que@you search the cage remains');
            await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.GATE_OF_IBAN) {
        ...
        if (flamesDone || stage === 7 || stage === 8 || stage === -1) {
            player.message("@que@from behind the door you hear cry's and moans");
            await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.GATE_OF_ZAMORAK) {
        player.message('@que@you open the huge wooden door');
        await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.SOULESS_CAGE_B) {
        ...
        player.message('@que@the cage is locked');
        await world.sleepTicks(3);
        ...
        if (souless) {
            // engage so npc.say has an interlocutor for this ambient taunt
            player.engage(souless);
            await souless.say('kuluf ali monopiate');
            player.disengage();
        }
        player.message('@que@you search through the bottom of the cage');
        await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.DEMONS_CHEST_CLOSED) {
        ...
        if (
            ...
            player.message('@que@the three amulets glow red in your satchel');
            await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.ZAMORAKIAN_TEMPLE_DOOR) {
        if (
            ...
            if (stage === 7 || dollComplete) {
                ...
                player.message(
                    '@que@@yel@Iban: who dares bring the witches magic into my temple'
                );
                await world.sleepTicks(3);
                player.message('@que@his eyes fixate on you as he raises his arm');
                await world.sleepTicks(3);
                player.message(
                    '@que@@yel@Iban: an imposter dares desecrate this sacred place..'
                );
                await world.sleepTicks(3);
                player.message(
                    '@que@@yel@Iban: ..home to the only true child of zamorak'
                );
                await world.sleepTicks(3);
                player.message('@que@@yel@Iban: join the damned, mortal');
                await world.sleepTicks(3);
                player.message('iban raises his staff to the air');
                player.message('@que@a blast of energy comes from ibans staff');
                await world.sleepTicks(3);
                player.message('you are hit by ibans magic bolt');
                player.sendTeleportBubble(player.x + 1, player.y, 'telegrab');
                player.damage(
                    Math.floor(player.skills.hits.current / 10) +
                        4 +
                        (Math.floor(Math.random() * 3) - 1)
                );
                player.message('@que@@yel@Iban:die foolish mortal');
                await world.sleepTicks(3);
                startIbanChamberHazard(player);
            } else {
                player.message('inside iban stands preaching at the alter');
            }
        } else {
            player.message('@que@The door refuses to open');
            await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.TOMB_OF_IBAN) {
        player.message('@que@you try to open the door of the tomb');
        await world.sleepTicks(3);
        player.message('but the door refuses to open');
        player.message('@que@you hear a noise from below');
        await world.sleepTicks(3);
    ...
    if (gameObject.id === IDS.FLAMES_OF_ZAMORAK) {
        player.message('@que@you search the stone structure');
        await world.sleepTicks(3);
...
async function onWallObjectCommandOne(player, wo) {
    ...
    if (wo.id === IDS.SPIDER_NEST_RAILING) {
        player.message('@que@you search the bars');
        await world.sleepTicks(3);
        const stage = getStage(player);
        if (player.cache.doll_of_iban || stage >= 7 || stage === -1) {
            player.message("@que@there's a gap big enough to squeeze through");
            await world.sleepTicks(3);
    ...
    if (wo.id === IDS.RAILING_168) {
        player.message('@que@the cage door has been sealed shut');
        await world.sleepTicks(3);
...
// wall-object command two (search a railing -> loose railing item)
async function onWallObjectCommandTwo(player, wo) {
    ...
    if (wo.id === IDS.RAILING_168) {
        player.message('@que@you search the cage');
        await world.sleepTicks(3);
...
// command two
// flames of zamorak 830 dispatches as command one (search is slot 0); unused here
