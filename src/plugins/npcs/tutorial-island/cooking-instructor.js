...
const COOKING_INSTRUCTOR_ID = 478;
const RAW_RAT_MEAT_ID = 503;
const COOKEDMEAT_ID = 132;
const HITS_MAX_THRESHOLD = 10;
...
async function onTalkToNPC(player, npc) {
    ...
    if (stage === 25) {
        ...
        if (!player.inventory.has(RAW_RAT_MEAT_ID)) {
            player.inventory.add(RAW_RAT_MEAT_ID, 1);
        ...
        await npc.say("Well I'm sure you'll get the hang of it soon", "Let's try again");
        ...
        if (
            player.inventory.has(COOKEDMEAT_ID) &&
            player.skills.hits.current < HITS_MAX_THRESHOLD
        ) {
            await npc.say('to eat the food left click on it in your inventory');
        } else {
            await npc.say(
                'There are many other sorts of food you can cook',
                'As your cooking level increases you will be able to cook ' +
                    'even more',
